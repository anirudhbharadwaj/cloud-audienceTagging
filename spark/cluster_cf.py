...
def filterByEventType(l):
	line=l.strip().split(",")
	ret=False
	if str(line[0].strip())=="301":
		ret=True
	return ret
...
if __name__ == "__main__":
	...
	s3Data = sc.textFile("s3n://clickstreamdata-cloud/data/*.txt")
	allData=s3Data.filter(filterByEventType)
	...
	totalUsers=onlyUser.count()
	clusterTotal=27
	if totalUsers<3:
		clusterTotal=1
	elif totalUsers>=3 and totalUsers<54:
		clusterTotal=int(totalUsers/3)
	elif totalUsers>=54 and totalUsers<270:
		clusterTotal=int(totalUsers/10)
	elif totalUsers>100000:
		k=int(totalUsers/1000)
		clusterTotal=int(totalUsers/(27*k))
	...
	clusters = KMeans.train(parsedData, clusterTotal, maxIterations=10, initializationMode="random",seed=50, initializationSteps=5, epsilon=1e-4)
	...
	sc.parallelize(overallRecc).saveAsTextFile("s3://anitest1/spark1/collabfilteroutputdemo/")
	sc.stop()